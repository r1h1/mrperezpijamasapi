...
namespace MrPerezApiCore.Data
{
    public class CategoriasData
    {
        ...
        public async Task<List<Categorias>> Lista()
        {
            ...
            {
                await con.OpenAsync();
                SqlCommand cmd = new SqlCommand("SELECT * FROM Categorias WHERE Estado = 1", con);
        ...
        public async Task<Categorias> Obtener(int Id)
        {
            ...
            {
                await con.OpenAsync();
                SqlCommand cmd = new SqlCommand("SELECT * FROM Categorias WHERE CategoriaId = @PCategoriaId AND Estado = 1", con);
